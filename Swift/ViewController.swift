...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.socket!.on("show_name",callback: showNameCallBack)
    ...
    @IBAction func PersonalSend(_ sender: AnyObject) {
        let person = self.nameTextField.text!
        let message = self.messageTextField.text!
        self.socket!.emit("personal message",person+"::"+message)
        self.messageTextField.text = ""
    }
    //lab2新增function
    ...
    func connectCallBack( data:[Any], ack:SocketAckEmitter)
    {
        ...
        self.socket!.emit("send_name", self.name)
        }
    }
    ...
    func showNameCallBack( data:[Any], ack:SocketAckEmitter)
    {
        let message: String = (data[0] as! String)
        
        let newString = message.replacingOccurrences(of: self.name, with: self.name+"<--")
        
        self.memberlist.text = "\(newString)"
    }
}
