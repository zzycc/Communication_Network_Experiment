...
io.on('connection',
      ...
      socket.on('personal message',
                ...
                for(var i=0 ; i<socketArray.length; i++){
                if(nameArray[i] == splitArray[0])
                receiveIndex = i;
                }
                socket.emit('show message on screen', nameArray[sendIndex]+" : "+splitArray[1]);
                if(receiveIndex!=-1){
                socketArray[receiveIndex].emit('show message on screen', nameArray[sendIndex]+" : "+splitArray[1]);
                }
                });
